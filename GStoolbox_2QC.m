function GStoolbox_2QC(fid,PATH,ref_path,ref_name,folder,mindepth,latlim,P,surface,Y, latbox, lonbox, closeplots)
%
...
if nargin < 13 || isempty(closeplots) || closeplots
    close all
end
...
data_PARAM={'TCARBN','ALKALI','OXYGEN','NITRAT','PHSPHT','SILCAT','SALNTY','CTDSAL','CTDOXY','PH_TOT','THETA','DOC','CFC_11','CFC_12','N2O'};
ref_PARAM={'GStco2','GStalk','GSoxygen','GSnitrate','GSphosphate','GSsilicate','GSsalinity','GSsalinity','GSoxygen','GSphts25p0','GStheta','GSdoc','GScfc11','GScfc12','GSn2o'};
param={'tco2','alk','oxygen','nitrate','phosphate','silicate','salinity','ctdsalinity','ctdoxygen','phts','theta','Doc','cfc11','cfc12','n2o'};
...
path=PATH;
    ...
    if isempty(latbox) || isempty(lonbox)
...
%     on occations there are data with flags other than 0,2,6 in the files which have to be removed here
    ...
    for p=P
        ...
        GSsec_QC(fid,path,ref_path,ref_name,ref_PARAM,input,paramrng,folder,p,expocode,ratio,latlim,mindepth,Y,minlat,maxlat,minlon,maxlon); 
        end    
    end
