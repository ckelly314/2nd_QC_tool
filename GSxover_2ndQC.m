function [diff, stdw] = GSxover_2ndQC(fid,path,parameter,paramrng,folder,expocode,mindepth,latlim,Y)
% 
...
mkdir([xoverresultsfolder filesep expocode filesep Y filesep num2str(mindepth) 'm_' num2str(latlim) 'degrees' filesep datestr(now,29) filesep],param{parameter});
plotname=cat(2,[xoverresultsfolder  filesep expocode filesep Y filesep num2str(mindepth) 'm_' num2str(latlim) 'degrees' filesep datestr(now,29) filesep param{parameter}],filesep,expo1,'___',expo2,'___',param{parameter},'___Xover');

else
mkdir([xoverresultsfolder  filesep folder filesep expocode filesep Y filesep num2str(mindepth) 'm_' num2str(latlim) 'degrees' filesep datestr(now,29) filesep],param{parameter});
plotname=cat(2,[xoverresultsfolder filesep folder filesep expocode filesep Y filesep num2str(mindepth) 'm_' num2str(latlim) 'degrees' filesep datestr(now,29) filesep param{parameter}],filesep,expo1,'___',expo2,'___',param{parameter},'___Xover');
end

diffmin=[-12 -10 0.8 0.8 0.8 0.8 -0.015 -0.015 0.8 -0.015 -1 0.8 0.8 0.8 0.8]; % lower x-limit for offset profile. columns are the different parameters
diffmax=[12 10 1.2 1.2 1.2 1.2 0.015 0.015 1.2 0.015  1 1.2 1.2 1.2 1.2]; % upper x-limit for offset profile. columns are the different parameters
stdmin=[2 2 0.005 0.01 0.01 0.01 0.002 0.002 0.005 0.005 0.005 0.01 0.01 0.01 0.01]; % prevents artificially small variability from skewing the analysis
addormul=[1 1 2 2 2 2 1 1 2 1 1 2 2 2 2]; % 1=additiive offset; 2=multiplicative offset
...
%% Calculate the offset for each pair of stations within the influence radius
   ...
   for i=1:length(ude),
       ...
       % CK update: reduce to 2 profiles for N2O
       %if length(F)< 5
       if length(F)< 2
          Ps=NaN;
          fprintf(fid,'2 profiles are necessary to calculate standard deviation of the profile difference.  There are only %d\n',length(F));
       end
      PARmean=[PARmean;Pm];
      PARstd=[PARstd;Ps];
      nnn=[nnn;length(F)];
  ...
  % CK: changed to a minimum of 1 station for N2O
  %if length(STAT1) < 3  | length(STAT2)<3 | length(PARmean)<1
  if length(STAT1) < 1  | length(STAT2)<1 | length(PARmean)<1
     disp('Too few data to make a X-over');
     fprintf(fid,'Too few data to make a X-over. Need at least one station; there are only %d in your cruise and %d in the reference cruise\n', length(STAT1), length(STAT2));
     orient tall;  
